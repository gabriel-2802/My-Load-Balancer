...
void loader_add_server(load_balancer_t *main, int server_id)
{
	...
	for (unsigned int i = 0; i < 3; ++i) {
		...
		acestuia va fi serverul de pe pozitia 0 */
...
char *loader_retrieve(load_balancer_t *main, char *key, int *server_id)
{
	*server_id = -1;
	...
	/* localizam serverul in care este stocat item-ul */
...
{
	/* vom marca fiecare servad eliberat din memorie pentru a nu fi
	apelata functia free_server_memory pe acelasi server de mai multe ori */
	...
	for (int i = 0; i < main->servers; ++i)
		if (!removed_servers[i]) {
			mark_removed_server(removed_servers, main->ring->servers_id[i],
								main->ring->servers_id, main->servers);
